...
import { Send, Sparkles, User, Bot, RefreshCw, HelpCircle, ArrowRight, CornerDownLeft } from 'lucide-react';

export default function ChatInterface({ datasetId, defaultSuggestions }) {
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      text: "Hello! I am your AI Data Analyst. Ask me anything about this dataset—such as key averages, top performers, monthly trends, missing values, or correlations.",
      data: null,
      chart: null,
      suggestions: [
  ...
  const handleSend = async (queryText) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: 'assistant',
          text: `Error processing question: ${err.response?.data?.detail || err.message}. Please try rephrasing.`,
          data: null,
          chart: null
        }
      ]);
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-xs flex flex-col h-[720px] overflow-hidden">
      {/* Header */}
      <div className="p-4 border-b border-slate-200 flex items-center justify-between bg-slate-50/50">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-800">Ask Your Data</h3>
            <p className="text-[11px] text-slate-500">Natural language questions safely mapped to Pandas operations</p>
          </div>
        </div>
          ...
          >
            ...
            >
              ...
              {msg.data && msg.data.length > 0 && typeof msg.data[0] === 'object' && (
                <div className="mt-3 overflow-x-auto border border-slate-200 rounded-lg max-h-48">
              ...
              {/* Plotly Chart Result */}
              {msg.chart && (
                <div className="mt-4 border border-slate-200 rounded-xl p-2 bg-white">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {msg.suggestions.map((sug, sIdx) => (
                      <button
                        ...
                        className="px-2.5 py-1 text-[11px] font-medium bg-slate-50 border border-slate-200 hover:border-blue-300 hover:bg-blue-50 text-slate-700 rounded-lg transition-all text-left flex items-center gap-1"
                      >
                        <span>{sug}</span>
        ...
        {loading && (
          <div className="flex gap-3 items-center">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl rounded-tl-xs px-4 py-3 text-xs text-slate-500 flex items-center gap-2 shadow-xs">
              ...
              <span>Executing safe Pandas operation...</span>
            </div>
          </div>
        )}
      ...
      <div className="p-4 border-t border-slate-200 bg-white">
        <div className="flex items-center gap-2">
          <input
            ...
            disabled={loading}
            className="flex-1 px-4 py-2.5 text-xs rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 disabled:opacity-60"
          />
          <button
            onClick={() => handleSend()}
            disabled={loading || !inputQuery.trim()}
            className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors shadow-xs disabled:opacity-50"
          >
